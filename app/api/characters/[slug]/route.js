...
export async function GET(req, { params }) {
  try {
    ...
    if(!character) {
      return new NextResponse('not-found', { sttaus: 404});
    }
    const characterQuotes = qoutes.data.filter(item => item.character_id === character.id);
    return NextResponse.json({ character, characterQuotes })
  } catch (error) {
